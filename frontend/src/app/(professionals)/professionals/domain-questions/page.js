...
import { saveDomainQuestionsProfessionals } from '@/utils/domainQuestionsApi';

function DomainQuestionsProfessionals() {
  ...
  const platformUsageOptions = [
    'Using AI to help me work faster and deliver better',
    'Learning and improving my marketing skills',
    'Building and sharing my professional portfolio',
    'Getting discovered by hirers and clients'
  ];

  // Step 2 options
  const marketIndustryOptions = [
    'FMCG / Consumer goods',
    'Fashion and retail',
    'Tech and startups (includes Web3)',
    'Finance and fintech',
  ...
  const handleNext = async () => {
    if (currentStep < totalSteps) {
      ...
      try {
        ...
        await saveDomainQuestionsProfessionals(user.id, formData);
  ...
  const canProceed = () => {
    ...
    if (currentStep === 2) return formData.marketIndustry.length > 0;
    if (currentStep === 3) return formData.stateRegion !== '';
    if (currentStep === 4) return formData.endConsumer.length > 0;
    return false;
  };
          ...
          <>
            <div className="mb-10 mt-[75px]">
              ...
              <p className="text-white/60 text-sm font-light leading-relaxed">
                Answer a few questions so we can configure your AI co-pilot for your market
              </p>
            </div>
            ...
            <form onSubmit={(e) => e.preventDefault()}>
              ...
              {currentStep === 2 && (
                <div className="animate-[fadeIn_0.4s_ease-in-out]">
                  <h2 className="text-xl font-medium text-white mb-3">
                    What market or industry do you primarily work in?
                  </h2>
                  <p className="text-sm text-white/50 mb-6">Select all that apply</p>
                  
                  <div className="space-y-3">
                    {marketIndustryOptions.map(option => {
                      const isSelected = formData.marketIndustry.includes(option);
                      const isOther = option === 'Other (please specify)';
                      
                      return (
                        <div key={option}>
                          <div
                            onClick={() => !isOther && toggleSelection('marketIndustry', option)}
                            className={`px-5 py-4 rounded-lg border-2 cursor-pointer transition-all text-sm font-medium ${
                              isSelected
                                ? 'bg-[#C19A4A]/20 border-[#C19A4A] text-white'
                                : 'bg-white/5 border-white/10 text-white/80 hover:border-white/30'
                          ...
                          {isOther && (
                            <input
                              ...
                              onChange={(e) => {
                                ...
                                setFormData(prev => ({ ...prev, otherIndustry: value }));
                                if (value.trim() && !formData.marketIndustry.includes('Other (please specify)')) {
                                  setFormData(prev => ({ ...prev, marketIndustry: [...prev.marketIndustry, 'Other (please specify)'] }));
                                } else if (!value.trim() && formData.marketIndustry.includes('Other (please specify)')) {
                                  setFormData(prev => ({ ...prev, marketIndustry: prev.marketIndustry.filter(t => t !== 'Other (please specify)') }));
                                }
                              }}
              ...
              {currentStep === 3 && (
                <div className="animate-[fadeIn_0.4s_ease-in-out]">
                  <h2 className="text-xl font-medium text-white mb-3">
                    What state or region do you operate in most?
                  </h2>
                  <p className="text-sm text-white/50 mb-6">Select one</p>
                  
                  <div className="space-y-3">
                    {stateRegionOptions.map(option => {
                      const isSelected = formData.stateRegion === option;
                      const needsInput = option === 'Outside Nigeria (please specify)';
                      
                      return (
                        <div key={option}>
                          <div
                            onClick={() => setFormData(prev => ({ ...prev, stateRegion: option }))}
                            className={`px-5 py-4 rounded-lg border-2 cursor-pointer transition-all text-sm font-medium ${
                              isSelected
                                ? 'bg-[#C19A4A]/20 border-[#C19A4A] text-white'
                                : 'bg-white/5 border-white/10 text-white/80 hover:border-white/30'
                          ...
                          {needsInput && isSelected && (
                            <input
                              type="text"
                              placeholder="Please specify location..."
                              value={formData.otherStateRegion}
                              onChange={(e) => {
                                setFormData(prev => ({ ...prev, otherStateRegion: e.target.value }));
                              }}
                              className="w-full mt-3 bg-transparent border border-white/20 rounded-lg px-4 py-3 text-sm text-white placeholder-white/40 focus:border-[#C19A4A] focus:ring-1 focus:ring-[#C19A4A] outline-none"
                            />
                          )}
              ...
              {currentStep === 4 && (
                <div className="animate-[fadeIn_0.4s_ease-in-out]">
                  <h2 className="text-xl font-medium text-white mb-3">
                    Who is the typical end consumer in your market?
                  </h2>
                  <p className="text-sm text-white/50 mb-6">Select all that apply</p>
                  
                  <div className="space-y-3">
                    {endConsumerOptions.map(option => {
                      const isSelected = formData.endConsumer.includes(option);
                      return (
                        <div
                          key={option}
                          onClick={() => toggleSelection('endConsumer', option)}
                          className={`px-5 py-4 rounded-lg border-2 cursor-pointer transition-all text-sm font-medium ${
                            isSelected
                              ? 'bg-[#C19A4A]/20 border-[#C19A4A] text-white'
                              : 'bg-white/5 border-white/10 text-white/80 hover:border-white/30'
...
export default DomainQuestionsProfessionals;
