...
function normalizeSkillName(name) {
  ...
  if (normalized.length < MIN_SKILL_NAME_LENGTH || normalized.length > MAX_SKILL_NAME_LENGTH) return null;
  ...
  proofs.forEach(proof => {
    ...
    if (Array.isArray(sae) && sae.length > 0) {
      sae.forEach(rawSkill => {
        ...
        if (existing) {
          ...
          skillMap.set(key, { name: normalizedName, proficiency: prof, categories: [cat], count: 1, confidences: [conf] });
        }
      });
    }
    ...
    if ((!sae || sae.length === 0) && Array.isArray(ed.skills) && ed.skills.length > 0) {
      ed.skills.forEach(skillName => {
        ...
        if (!skillMap.has(key)) {
          skillMap.set(key, { name: normalized, proficiency: 'mentioned', categories: ['unknown'], count: 1, confidences: [0.5] });
        }
      });
    }
  });
...
const Portfolio = () => {
    ...
    const loadPortfolioData = async () => {
      try {
        ...
        if (viewingUserId) {
          ...
          if (!userProfile) { setError('User not found'); setLoading(false); return; }
          setProfile(userProfile);
          setUser({ id: viewingUserId, email: userProfile.users?.email });
          const userProofs = await getUserProofs(viewingUserId);
          setProofs(userProofs || []);
        } else {
          const currentUser = await getCurrentUser();
          if (!currentUser) { setError('Please log in to view your portfolio'); setLoading(false); router.push('/login'); return; }
          setUser(currentUser);
  ...
  const getPortfolioUrl = () => {
    ...
    if (userId) return `${window.location.origin}/portfolio?id=${userId}`;
              ...
              <div className="relative p-[2px] rounded-2xl bg-gradient-to-br from-[#C19A4A] via-[#d9b563] to-blue-500">
                <div className="bg-[#111625] rounded-xl p-5">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {[1,2,3,4,5].map(i => <div key={i} className="h-7 bg-gray-700/50 rounded-full w-20"></div>)}
                  </div>
                </div>
              </div>
            </div>
    ...
    <div className="min-h-screen bg-[#0B0F1B] [background-image:radial-gradient(ellipse_80%_40%_at_50%_0%,#C19A4A1A_0%,transparent_100%),radial-gradient(ellipse_60%_30%_at_90%_100%,#C19A4A12_0%,transparent_100%)] text-white pt-24 pb-28 px-5">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="lg:grid lg:grid-cols-[1fr_400px] lg:gap-6">
          ...
          {/* File Viewer Modal */}
          {selectedFile && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm" onClick={() => setSelectedFile(null)}>
              <div className="relative max-w-[90vw] max-h-[85vh] bg-[#111625] rounded-2xl overflow-hidden shadow-2xl border border-white/10" onClick={e => e.stopPropagation()}>
                <div className="flex items-center justify-between px-4 py-3 border-b border-white/10 bg-[#0d1020]">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-semibold text-white truncate max-w-[200px]">{selectedFile.proof_name || 'Document'}</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <a href={selectedFile.file_ipfs_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-500/10 border border-blue-500/20 text-blue-400 text-[11px] font-semibold hover:bg-blue-500/20 transition-colors">
                      <ExternalLink size={11} /> Open
                    </a>
                    <button onClick={() => setSelectedFile(null)} className="w-8 h-8 flex items-center justify-center rounded-full bg-white/5 text-gray-400 hover:text-white hover:bg-white/10 transition-colors">
                      <X size={14} />
                    </button>
                  </div>
                </div>
                {selectedFile.file_ipfs_url?.match(/\.(jpg|jpeg|png|gif|webp)$/i) ? (
                  <img src={selectedFile.file_ipfs_url} alt="Proof document" className="max-h-[75vh] w-auto object-contain" />
                ) : selectedFile.file_ipfs_url?.match(/\.pdf$/i) ? (
                  <iframe src={selectedFile.file_ipfs_url} className="w-[85vw] h-[75vh] max-w-4xl" title="Proof PDF" />
                ) : (
                  <div className="p-12 text-center text-gray-400 flex flex-col items-center gap-3">
                    <FileCheck size={32} className="text-gray-600" />
                    <p className="text-sm">Preview not available for this file type.</p>
                    <a href={selectedFile.file_ipfs_url} target="_blank" rel="noopener noreferrer" className="text-blue-400 text-xs hover:underline flex items-center gap-1">
          ...
          {/* Metadata JSON Modal */}
          {selectedProof && (
            <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm px-4" onClick={() => setSelectedProof(null)}>
              <motion.div
                    ...
                    <div className="flex items-center justify-between px-5 py-4 border-b border-white/10 shrink-0">
                      <div className="flex items-center gap-3 min-w-0">
                        ...
                        <div className="min-w-0">
                          ...
                          <p className="text-[10px] text-gray-500 uppercase tracking-wider">{selectedProof.metadata_ipfs_url ? 'IPFS Metadata JSON' : 'Extracted Data'}</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-2 shrink-0">
                        {selectedProof.extracted_data && (
                          <button onClick={() => navigator.clipboard.writeText(JSON.stringify(selectedProof.extracted_data, null, 2))} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-[#C19A4A]/10 border border-[#C19A4A]/20 text-[#C19A4A] text-[11px] font-semibold hover:bg-[#C19A4A]/20 transition-colors">
                            <Copy size={12} /> Copy
                          </button>
                        )}
                        {selectedProof.metadata_ipfs_url && (
                          <a href={selectedProof.metadata_ipfs_url} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg bg-blue-500/10 border border-blue-500/20 text-blue-400 text-[11px] font-semibold hover:bg-blue-500/20 transition-colors">
                            <ExternalLink size={12} /> IPFS
                          </a>
                        )}
                        <button onClick={() => setSelectedProof(null)} className="w-8 h-8 flex items-center justify-center rounded-full bg-white/5 text-gray-400 hover:text-white hover:bg-white/10 transition-colors">
                          <X size={14} />
                        </button>
                      </div>
                    </div>
              ...
              <div className="bg-[#151925] rounded-2xl p-6">
                <div className="flex items-start gap-4 mb-6">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-gray-300 text-sm leading-relaxed">{profile?.bio || 'Building the future of decentralized applications.'}</p>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap items-center gap-3 text-xs">
                  {profile?.social_links?.twitter && (
                    <a href={profile.social_links.twitter.startsWith('http') ? profile.social_links.twitter : `https://twitter.com/${profile.social_links.twitter}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 bg-[#0B0F1B] rounded-md px-3 py-1.5 hover:bg-[#0B0F1B]/80 transition-colors">
                      <FontAwesomeIcon icon={faXTwitter} className="w-[14px] h-[14px] text-[#C19A4A]" />
                    </a>
                  )}
                  {profile?.social_links?.linkedin && (
                    <a href={profile.social_links.linkedin.startsWith('http') ? profile.social_links.linkedin : `https://linkedin.com/in/${profile.social_links.linkedin}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 bg-[#0B0F1B] rounded-md px-3 py-1.5 hover:bg-[#0B0F1B]/80 transition-colors">
                      <FontAwesomeIcon icon={faLinkedinIn} className="w-[14px] h-[14px] text-[#C19A4A]" />
                    </a>
                  )}
                  {profile?.social_links?.github && (
                    <a href={profile.social_links.github.startsWith('http') ? profile.social_links.github : `https://github.com/${profile.social_links.github}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 bg-[#0B0F1B] rounded-md px-3 py-1.5 hover:bg-[#0B0F1B]/80 transition-colors">
                      <FontAwesomeIcon icon={faGithub} className="w-[14px] h-[14px] text-[#C19A4A]" />
                    </a>
                  )}
                  {profile?.social_links?.website && (
                    <a href={profile.social_links.website.startsWith('http') ? profile.social_links.website : `https://${profile.social_links.website}`} target="_blank" rel="noopener noreferrer" className="flex items-center gap-1.5 bg-[#0B0F1B] rounded-md px-3 py-1.5 hover:bg-[#0B0F1B]/80 transition-colors">
              ...
              <div className="bg-[#111625] rounded-xl p-5">
                ...
                <div className="flex flex-wrap gap-2">
                  {skills.length > 0 ? skills.map((skill, index) => (
                    <span key={index} className="text-xs text-[#C19A4A] bg-[#1A1F2E] border border-[#C19A4A]/20 px-3 py-1.5 rounded-full hover:bg-[#C19A4A]/10 transition-colors">
                      {skill.name}
                    </span>
                  )) : (
        ...
        <div className="mt-8">
              ...
              {filterTabs.map(filter => {
                ...
                return (
                  <button
                    ...
                    className={`px-6 py-2 rounded-full text-sm font-medium transition-all whitespace-nowrap flex items-center gap-2 ${activeFilter === filter ? 'bg-[#C19A4A] text-[#0B0F1B]' : 'bg-[#151925] text-white hover:bg-[#1a1f2e]'}`}
                  >
                    {filter}
                    <span className={`text-xs px-2 py-0.5 rounded-full ${activeFilter === filter ? 'bg-[#0B0F1B]/20 text-[#0B0F1B]' : 'bg-white/10 text-gray-400'}`}>
                      {count}
                    </span>
                  </button>
                );
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 mb-6">
            <p className="text-gray-400 text-sm">Showing {sortedProofs.length} proofs — {activeFilter === 'All' ? 'All Work' : activeFilter}</p>
            <select value={sortBy} onChange={e => setSortBy(e.target.value)} className="bg-[#151925] text-white px-4 py-2 rounded-lg border border-[#C19A4A]/30 text-sm cursor-pointer w-full sm:w-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
            {sortedProofs.length === 0 ? (
              <div className="bg-[#151925] rounded-2xl p-12 text-center border border-[#C19A4A]/20">
                ...
                <button onClick={() => router.push('/upload')} className="px-6 py-3 bg-[#C19A4A] text-[#0B0F1B] rounded-lg font-semibold hover:bg-[#d4af37] transition-colors">
                  Upload Your First Proof
                </button>
              </div>
            ) : (
              sortedProofs.map(proof => (
                <div key={proof.id} className="bg-[#151925] rounded-2xl overflow-hidden border border-[#C19A4A]/20 hover:border-[#C19A4A]/50 transition-all">

                  {/* Proof Preview — video embed, image, PDF, or fallback */}
                  {(proof.reference_link && isVideoUrl(proof.reference_link))
                    ? (
                      <div className="relative h-64 bg-[#0B0F1B] overflow-hidden">
                        <iframe
                          src={`${getVideoEmbedUrl(proof.reference_link)}?modestbranding=1&rel=0`}
                        ...
                        <span className="absolute top-4 left-4 inline-flex items-center gap-2 px-3 py-1.5 bg-[#C19A4A] text-[#0B0F1B] rounded-full text-xs font-bold">
                          ...
                          {proof.proof_type?.replace(/_/g, ' ').split(' ').map(w => w.charAt(0).toUpperCase() + w.slice(1)).join(' ')}
                        </span>
                      </div>
                    )
                    : (proof.file_ipfs_url || proof.files?.[0]?.file_url) && (
                      <div className="relative h-64 bg-gradient-to-br from-[#1a1f2e] to-[#0B0F1B] overflow-hidden">
                        {(proof.files?.[0]?.mime_type?.includes('image') || proof.file_ipfs_url?.match(/\.(jpg|jpeg|png|gif|webp)$/i)) ? (
                          <img src={proof.files?.[0]?.file_url || proof.file_ipfs_url} alt={proof.proof_name} className="w-full h-full object-cover" />
                  ...
                  <div className="p-5">
                    ...
                    <div className="flex items-center gap-4 mb-3 text-sm">
                      ...
                      {proof.status === 'portfolio_only' && (
                        <span className="flex items-center gap-1 text-gray-400 text-sm">
                          <i className="fa-solid fa-folder text-xs"></i> Portfolio only
                        </span>
                      )}
                    </div>
                    ...
                    {proof.extracted_data?.smart_tags && proof.extracted_data.smart_tags.length > 0 && (
                      <div className="flex flex-wrap gap-2 mb-4">
                        {proof.extracted_data.smart_tags.slice(0, 5).map((tag, idx) => (
                          <span key={idx} className="inline-flex items-center px-2.5 py-1 bg-[#C19A4A]/10 border border-[#C19A4A]/30 rounded-full text-[#C19A4A] text-[10px] font-medium">
                            {tag}
                          </span>
                        ))}
                    ...
                    <div className="bg-[#0B0F1B] rounded-lg p-3 border border-[#C19A4A]/20">
                      <div className="flex items-center justify-between gap-2">
                        <div className="flex items-center gap-2 flex-1 min-w-0">
                          ...
                          {proof.blockchain_tx ? (
                            <a href={`https://solscan.io/tx/${proof.blockchain_tx}?cluster=devnet`} target="_blank" rel="noopener noreferrer" className="text-[#C19A4A] font-mono text-xs hover:underline truncate">
                        ...
                        <div className="flex items-center gap-2 flex-shrink-0">
                          {(proof.file_ipfs_url || proof.files?.[0]?.file_url) && (
                            <button
                              onClick={() => setSelectedFile({ file_ipfs_url: proof.files?.[0]?.file_url || proof.file_ipfs_url, mime_type: proof.files?.[0]?.mime_type, proof_name: proof.proof_name })}
                          ...
                          {(proof.metadata_ipfs_url || proof.extracted_data) && (
                            <button
                              ...
                              className="flex items-center gap-1 px-2 py-1.5 rounded transition-colors shrink-0 text-[#C19A4A] bg-[#C19A4A]/10 border border-[#C19A4A]/20 hover:bg-[#C19A4A]/20"
                          ...
                          <button
                            onClick={() => { if (window.confirm('Are you sure you want to delete this proof?')) { console.log('Delete proof:', proof.id); } }}
        ...
        {/* Floating action bar */}
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.5 }} className="fixed bottom-2 left-3 right-3 z-50">
          <div className="w-full max-w-7xl mx-auto relative p-[2px] rounded-xl bg-gradient-to-r from-[#C19A4A] via-[#d9b563] to-blue-500">
            <div className="bg-[#1C1C1C]/95 backdrop-blur-md p-2 rounded-xl shadow-2xl flex items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button onClick={handleSharePortfolio} className="px-3 py-2 rounded-lg border border-[#C19A4A] text-[#C19A4A] text-xs font-semibold hover:bg-[#C19A4A]/10 transition-colors flex items-center gap-1.5">
                  {copied ? (
                    <><CheckCircle size={14} /><span className="hidden sm:inline">Copied!</span><span className="sm:hidden">Copied!</span></>
                  ) : (
                    <><Share2 size={14} /><span className="hidden sm:inline">Share Portfolio</span><span className="sm:hidden">Share</span></>
                  )}
                </button>
                <button onClick={() => router.push('/upload')} className="px-3 py-2 rounded-lg bg-gradient-to-r from-[#C19A4A] to-[#d9b563] text-black text-xs font-semibold hover:shadow-[0_0_20px_rgba(193,154,74,0.4)] transition-all flex items-center gap-1.5">
                  <Plus size={14} />
        ...
        <div className="fixed bottom-24 right-6 z-50">
          <div className="relative">
            ...
            >
              <Player autoplay loop src="/assets/animations/lottie_transparent.json" style={{ width: 100, height: 100 }} />
            </button>
          </div>
        </div>
      </div>
